...
describe('Simulation: nft winner price > ticketSaleFunds', async () => {
    let manager;
    let player;
    let managerAddress;
    let playerAddress;
    ...
    // simulation
    const maxPrice = ethers.utils.parseUnits('2', decimals);
    collections[0].price = maxPrice;
    const simulatedTotalSales = ethers.utils.parseUnits('0.1', decimals);
    const investor1Amount = maxPrice.div(2);
    const investor2Amount = maxPrice.mul(2);
    ...
    it("Withdraw by house (ntf market price is below than the defined one)", async () => {
        let wonPrize = await GameContract.wonPrize();
        // simulate the market price to be below the defined one
        wonPrize = wonPrize.price.sub(1000);
