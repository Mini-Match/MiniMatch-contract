...
describe('Simulation: extreme case: investments are negligible small comparing to the sales', async () => {
    let manager;
    let player;
    let managerAddress;
    let playerAddress;
    ...
    const simulatedTotalSales = ethers.utils.parseUnits('1', decimals).mul(7998);
    const investor1Amount = ethers.utils.parseUnits('1', decimals);
    const investor2Amount = ethers.utils.parseUnits('2', decimals);
    ...
    it("Withdraw by house (ntf market price is equal to the defined one)", async () => {
        ...
        // investmentorProftAfterProportion = (sales - wonPrize) * 60% * (totalInvestedAmount/(totalInvestedAmount + ticketSaleFunds))
        const investmentorProftAfterProportion = 
        ticketSaleFunds.
        sub(wonPrize).
        mul(INVESTORS_PERCENT_DISTRIBUTION).
        mul(totalInvestedAmount).
        div(totalInvestedAmount.add(ticketSaleFunds)).
        div(100);

        const houseProfit = ticketSaleFunds.sub(wonPrize).sub(investmentorProftAfterProportion);
