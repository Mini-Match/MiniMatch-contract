...
describe('Simulation: investmentFunds  + ticketSaleFunds exactly equal maxPrizePrice', async () => {
    let manager;
    let player;
    let managerAddress;
    let playerAddress;
    ...
    // simulation
    const maxPrice = ethers.utils.parseUnits('3', decimals);
    collections[0].price = maxPrice;
    const simulatedTotalSales = ethers.utils.parseUnits('1', decimals);
    const investor1Amount = ethers.utils.parseUnits('1', decimals);
    const investor2Amount = ethers.utils.parseUnits('1', decimals);
